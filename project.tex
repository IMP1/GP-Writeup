...
%TODO ~ADD, ~MENTION: How to find out what nodes have been added/matched.
%TODO ~MENTION: Using the output graph as the new input graph.
%TODO ~ADD: More details about steps of different sizes. Maybe it'd just be instead of `if current_step == this { finalise; }` it would be something more like `if current_step == this && step_size == this { finalise; }`?
% With step_size being an enum, that's read in from the arguments (args can either be ints or strings, but are converted to the enum when they're read in).

% Factor out the finalisation, as it can happen at other parts of the code now.

% There are certain additional things we need to know at the beginning of the running of a graph program on a host graph:
...
% Using these, we will do the following:
...
% User has to (optionally) give certain parameters. By default, the compiler does it's normal thing.
% If a step size is specified, only that much is run.

% Give the GP2-run some usage string, and show that.
% 
...
% Maintainability
	% Is good. I didn't distrupt things too much. I was highly coupled with existing implemetations though.
% Usability?
